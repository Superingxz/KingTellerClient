 ...
 */
public class QRDeliveryMaterialActivity extends Activity implements
	...
	private List<QRDeliveryBean> mDeliveryBeanList = new ArrayList<QRDeliveryBean>();
	private User user;
	private int mPage = 1;
	private boolean isFirst = true;
	private int onItemNum = 0;

	private SearchGeneraItem qrFhwl_search = new SearchGeneraItem();
	...
	public void onCreate(Bundle paramBundle) {
		...
		mContext = QRDeliveryMaterialActivity.this;
		initUI();
		initData();
	}
	...
	private void initUI() {
		...
		adpater = new QRDeliveryReceiptAdapter(mContext, new ArrayList<QRDeliveryBean>(), null, null, null, null, null, 1);
	...
	public void onItemClick(AdapterView<?> parent, View view, int position,
			long id) {
		QRDeliveryBean delivertbean = (QRDeliveryBean) parent.getAdapter().getItem(position);
		onItemNum = position - 1;
		Intent intent = new Intent(this, QRCargoScanActivity.class);
		intent.putExtra("DelivertBean", (QRDeliveryBean) delivertbean); 
		intent.putExtra("Type", "1");
		startActivityForResult(intent, KingTellerStaticConfig.QRCODE_FHSM_BACK); 
	}
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data)
    {
        ...
        switch (requestCode) {
		case KingTellerStaticConfig.QRCODE_FHSM_BACK:
			if (resultCode == RESULT_OK) {
				String mFH_wlzs = (String) data.getSerializableExtra("mFH_wlzs");
				String mFH_tmzs = (String) data.getSerializableExtra("mFH_tmzs");
				mDeliveryBeanList.get(onItemNum).setWlTotal(mFH_wlzs);
				mDeliveryBeanList.get(onItemNum).setBarcodeCount(mFH_tmzs);
				adpater.setListsdelivery(mDeliveryBeanList);
			}
			break;
		default:
			break;
	...
	public void onClick(View v) {
		...
		case R.id.layout_main_righttwo_btn:
			...
			final SearchGeneralDialog searchgeneral_dialog = new SearchGeneralDialog(mContext, R.style.Login_dialog, 6, new Gson().toJson(str));
			searchgeneral_dialog.setOnBtnSearchClickL(
				new OnBtnSearchClickL() {
	                ...
	                public void onBtnClick(String ss) {
	                	...
	                	qrFhwl_search = KingTellerJsonUtils.getPerson(ss, SearchGeneraItem.class);
	                	mPage = 1;
	                	isFirst = true;
	                	getQRDelivertyData();
	                }
	...
	public void getQRDelivertyData() {
		...
		if(!KingTellerJudgeUtils.isEmpty(qrFhwl_search.getQrFhwldh())){
			params.put("fbillNo", qrFhwl_search.getQrFhwldh());//发货单编号 
		}

		fh.post(KingTellerConfigUtils.CreateUrl(this, KingTellerUrlConfig.CkdLists),params, 
				new AjaxHttpCallBack<List<QRDeliveryBean>>(this,
						new TypeToken<List<QRDeliveryBean>>() {
						}.getType(), true) {
					...
					@Override
					public void onDo(List<QRDeliveryBean> data) {
						if (data.size() > 0 && data.get(0).getCode().equals("")) {
							if(mPage == 1){
								mDeliveryBeanList = data;
								adpater.setListsdelivery(mDeliveryBeanList);
								getListviewObj().setStatus(LoadingEnum.LISTSHOW);
								getListviewObj().getListview().setLoadMoreEnabled(true);
							}else{
								adpater.addListsdelivery(data); 
							}
						}else {
							if(mPage == 1){
