...
//选择这一行时，选中radio
function selectThisRowOrgTopList(obj,id,name) {
	...
	function queryPageOrgTopList(redirect) {
		...
		getTableDataOrgTopList(
					"myTable",
					appContext+"/saleCenter/saleBill/orgTopList_salesMobile.action;jsessionid="+sessionId,
					"orgName=" + encodeURI(encodeURI($.trim($(".modal-body #orgName").val()))) + "&pageNum="+currentPage);
		
	}

	//搜索
	function searchOrgTopList() {
		getTableDataOrgTopList(
					"myTable",
					appContext+"/saleCenter/saleBill/orgTopList_salesMobile.action;jsessionid="+sessionId,
					"orgName=" + encodeURI(encodeURI($.trim($(".modal-body #orgName").val()))));
	}
	...
	//加载数据
	var currentPage ;
	function getTableDataOrgTopList(tableId, url, param) {
		//alert(tableId+','+url+','+param);
		$.jsonp({
			...
			success : function(msg) {//msg为返回的数据，在这里做数据绑定
				...
				for ( var i = 0; i < data.length; i++) {
					generateTrOrgTopList(data[i].orgId, data[i].orgName,tableId);
				}
	...
	//生成表格行
	function generateTrOrgTopList(id, name,tableId) {

		var tr = $("<tr style='cursor: pointer;'></tr>");
		tr.bind("click", function() {
			selectThisRowOrgTopList(this, id, name);
		})
