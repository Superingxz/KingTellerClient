...
*/
$(function() {
//	var uploadButton='<div style="position: absolute;margin-left: 80px"><div id="uploadFile-button" class="uploadify-button " '
//		+'style="height: 30px; line-height: 30px; width: 100px;cursor:pointer;">'+
//		'<span class="uploadify-button-text" onclick="$(\'#uploadFile\').uploadify(\'upload\',\'*\'),uploadOnline()">开始上传</span></div></div>';
//	$('#u_a').append(uploadButton);
	
	/*var file='<input type="file" id="uploadFile" name="file" />';
	$('#u_a').append(file);
	var fileQueue='<div id="file_queue" name="file_queue" style="width: 350px; border: 1px; float: left; margin-top: 5px"></div>';
	$('#u_a').append(fileQueue);
	*/
	var hidden = '<div align="center" style="display:none;"><form action="#" method="post" id="fhidden" name="fhidden"><input type="hidden" id="fileNum" name="fileNum" value="" /></form><textarea id="uploadText" name="uploadText" ></textarea></div>';
	$('#u_a').append(hidden);
});
		...
		init : function(options, swfUploadOptions) {
			...
			return this.each(function() {
				...
				if (flashInstalled) {
					...
					if($('#upload_online').length>0){
						$wrapper.append('<div id="file_inner" style="position: absolute;margin-left: 160px"><div id="uploadFile-button-a"  '
								+'style="height: 30px; line-height: 30px; width: 100px;cursor:pointer;">'+
								'<input type="button" onclick="uploadOnline(),$(\'#uploadFile\').uploadify(\'upload\',\'*\')" value="上传文件"></input></div></div>');
					}else{
						$wrapper.append('<div style="position: absolute;margin-left: 90px"><div id="uploadFile-button-a"  '
								+'style="height: 30px; line-height: 30px; width: 100px;cursor:pointer;">'+
								'<input type="button" onclick="setDir(),$(\'#uploadFile\').uploadify(\'upload\',\'*\')" value="上传文件"></input></div></div>');	
					}
					...
					$button.html('<input type="button" id="button-text" value="'+settings.buttonText+'"></input>')
					.css({
						'height'      : settings.height + 'px',
				...
				if (args[0]) {
					...
					if (args[0] == '*') {
						...
						for (var n = 0; n < args.length; n++) {
							...
							if($('#' + args[n]).find('.data').text().indexOf("上传完成")>=0){
								var text = $('#uploadText').val();
								var mark1 = $('#' + args[n]).find('span.mark1').text();
								var mark2 = $('#' + args[n]).find('span.mark2').text();
								var sub_prefix = text.indexOf('{"id":"'+mark1);
								var sub_suffix = text.indexOf(mark2+'"},');
								var new_text = text.substring(0, sub_prefix)+text.substring(sub_suffix+(mark2+'"},').length,text.length);
								$('#uploadText').html(new_text);
							}else{
								var isJudge = $('#file_upload_judge').val();
								if(isJudge!=null&&isJudge=="isImage"){
									var f_img = $('#' + args[n]).find('span.file_img').text();
									if(f_img=="f_d"){
										var mark1 = $('#' + args[n]).find('span.mark1').text();
										deleteUploadFile(mark1);
									}
								}								
							}
							...
							$('#' + args[n]).remove();
							if(file_upload_queue_size>0){
								file_upload_queue_size = file_upload_queue_size-1;								
							}
						}
					}
				} else {
			...
			this.each(function() {
				...
				if (args[0]) {
					if (args[0] == '*') {
						...
						if(file_upload_num+swfuploadify.queueData.queueLength==0){
							alert('请选择要上传的文件');
						}
	...
	var handlers = {
		...
		onSelect : function(file) {
			...
			for (var n in this.queueData.files) {
				...
				if (queuedFile.uploaded != true && queuedFile.name == file.name) {
					var replaceQueueItem = confirm('文件 "' + file.name + '" 已经在上传列表中，你确定还要重新覆盖？');
					if (!replaceQueueItem) {
			...
			if(file_upload_queue_size==0){
				file_upload_queue_size = this.queueData.queueLength+1;	
			}else{
				file_upload_queue_size = file_upload_queue_size +1;
			}
			...
			if ($.inArray('onSelectError', settings.overrideEvents) < 0) {
				switch(errorCode) {
					...
					case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
						this.queueData.errorMsg += '\n上传的文件 "' + file.name + '" 大小超过上传的限制 .';
						break;
					case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
						this.queueData.errorMsg += '\n上传的文件： "' + file.name + '" 是空文件.';
						break;
					case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
		...
		onUploadSuccess : function(file, data, response) {
			...
			if ($.inArray('onUploadSuccess', settings.overrideEvents) < 0) {
				...
				}else if(isJudge!=null&&isJudge=="isList"){
					//$('#' + file.id).find('.data').remove();
				...
				file_upload_success_div_id = file.id; 
			}
