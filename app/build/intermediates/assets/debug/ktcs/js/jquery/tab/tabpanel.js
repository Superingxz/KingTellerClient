...
TabPanel = function(config){
  ...
  this.tabWidth = config.tabWidth||110;//样式.tabpanel_mover li中width+4
...
TabPanel.prototype = {
  ...
  init : function(){
    ...
    if(this.widthResizable || this.heightResizable){
  	  ...
  	  this.render.parent().resize(function(){
  	    tabEntity.resize();
  	  });
    }
    ...
    //如果容器已经有tabpanel，则不需要在创建
    var appendPos="after";
    if(this.render.children(".tabpanel").length>0){
    	this.tabpanel=jQuery(this.render.children(".tabpanel")[0]);
    ...
    if(appendPos=='after'){
    	...
    	this.tabpanel_tab_content.prependTo(this.tabpanel);
    }
    ...
    //实例化内容层
    if(this.tabpanel.children(".tabpanel_content").length>0){
    	this.tabpanel_content=jQuery(this.tabpanel.children(".tabpanel_content")[0]);  	
  ...
  showScroll : function(){
    ...
    var liWidth =0;//= this.tabpanel_mover.children().length*this.tabWidth;
    //计算LI总宽度(tql modify)
    for(var i=0;i<this.tabs;i++){
    	liWidth+=this.tabs[i].width();
    }
  ...
  addTab : function(tabitem){
    ...
    else if(this.scrollFinish)
    {
      ...
      //添加content元素
      var content =null;
      var contentArr=this.tabpanel_content.find('div[tabId="'+tabitem.id+'"]');
      ...
      //根据每个tab设定的宽度设置li的宽度
      if(tabitem.width!=null&&tabitem.width>0){
    	  tab.width(tabitem.width);
      }
      ...
      tab.bind('click', function(position){
        return function(){
          ...
          //扩展一个激活tab事件
          if(tabInst.onActiveTab!=null){
         	  tabInst.onActiveTab(tabitem.id);
          }
        };
      }(this.tabs.length-1));
  ...
  kill : function(position){
    ...
    var content=this.tabs[position].content
    var child_frame = content.find('iframe');
    if(child_frame){
    	//child_frame.remove();
    }
